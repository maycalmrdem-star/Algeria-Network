      ...
      <div className="p-6">
        {activeTab === 'automod' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <Toggle label="منع الروابط (Anti-Links)" checked={automod.antiLinks} onChange={(e) => setAutomod({...automod, antiLinks: e.target.checked})} />
            <Toggle label="منع السبام (Anti-Spam)" checked={automod.antiSpam} onChange={(e) => setAutomod({...automod, antiSpam: e.target.checked})} />
            <Toggle label="منع الكلمات البذيئة (Anti-Bad Words)" checked={automod.antiBadWords} onChange={(e) => setAutomod({...automod, antiBadWords: e.target.checked})} />
            ...
            {automod.antiBadWords && (
              <div>
                ...
                <textarea 
                  ...
                  onChange={(e) => setAutomod({...automod, badWordsList: e.target.value})}
        ...
        {activeTab === 'welcome' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <Toggle label="تفعيل الترحيب" checked={welcome.enabled} onChange={(e) => setWelcome({...welcome, enabled: e.target.checked})} />
            
            {welcome.enabled && (
              <>
                <Input label="أيدي روم الترحيب (Channel ID)" value={welcome.channelId} onChange={(e) => setWelcome({...welcome, channelId: e.target.value})} />
                <div>
                  <label className="block text-sm text-gray-400 mb-2">رسالة الترحيب</label>
                  <textarea 
                    value={welcome.messageText}
                    onChange={(e) => setWelcome({...welcome, messageText: e.target.value})}
                    className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white h-24"
                  />
                  <p className="text-xs text-gray-500 mt-1">المتغيرات: [user], [server], [memberCount]</p>
                </div>
                <Input label="رابط صورة الترحيب" value={welcome.imageUrl} onChange={(e) => setWelcome({...welcome, imageUrl: e.target.value})} dir="ltr" />
              </>
            )}
        ...
        {activeTab === 'tickets' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <Input label="أيدي الكاتيجوري (Category ID) لفتح التذاكر فيه" value={tickets.categoryId} onChange={(e) => setTickets({...tickets, categoryId: e.target.value})} />
            <Input label="أيدي رتبة الدعم الفني (Support Role ID)" value={tickets.adminRoleId} onChange={(e) => setTickets({...tickets, adminRoleId: e.target.value})} />
